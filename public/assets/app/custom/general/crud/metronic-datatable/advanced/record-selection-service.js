...
var KTDatatableMealTypes = function () {
    ...
    var options = {
        ...
        data: {
            ...
            source: {
                read: {
                    url: `${location.protocol}//${window.location.host}/service/getall`,
                },
            },
            pageSize: 10,
            serverPaging: true,
        ...
        columns: [
            ...
            },
            {
                ...
                template: function (row) {
                    return '\
                    \<a href="' + location.protocol + "//" + window.location.host + '/service/edit/' + row._id + '" class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="Edit">\
    ...
    var mealTypesSelector = function () {
        ...
        $('#kt_form_status,#kt_form_category').selectpicker();
        ...
        $(document).on('click', '.KTStatusUpdate', function () {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `${window.location.protocol}//${window.location.host}/service/status-change/${elemID}`;
                }
            });
        })
        $(document).on('click', '.ktDelete', function () {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `${location.protocol}//${window.location.host}/service/delete/${elemID}`;
                }
            });
        });
    };
