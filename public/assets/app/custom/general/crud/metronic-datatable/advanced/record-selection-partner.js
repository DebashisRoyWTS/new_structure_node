...
var KTDatatableMealTypes = function () {
    ...
    var options = {
        ...
        data: {
            ...
            source: {
                read: {
                    url: `${location.protocol}//${window.location.host}/partner/getall`,
                },
            },
            pageSize: 10,
            serverPaging: true,
        ...
        columns: [
            {
                ...
                template: function (row) {
                    if (row.image) {
                        return `<img style="max-height: 45px" src = "${window.location.protocol}//${window.location.host}/uploads/partner/${row.image}">`;
                    } else {
            ...
            },
            {
                ...
                template: function (row) {
                    return '\
                    \<a href="' + location.protocol + "//" + window.location.host + '/partner/edit/' + row._id + '" class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="Edit">\
    ...
    var mealTypesSelector = function () {
        ...
        $(document).on('click', '.KTStatusUpdate', function () {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `${window.location.protocol}//${window.location.host}/partner/status-change/${elemID}`;
                }
            });
        })
        $(document).on('click', '.ktDelete', function () {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `${location.protocol}//${window.location.host}/partner/delete/${elemID}`;
                }
            });
        });
    };
