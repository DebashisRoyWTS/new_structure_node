...
var KTDatatableMealTypes = function () {
    ...
    var options = {
        ...
        data: {
            ...
            source: {
                read: {
                    url: `${location.protocol}//${window.location.host}/product-category/getall`,
                },
            },
            pageSize: 10,
            serverPaging: true,
        ...
        columns: [
            ...
            },
            {
                ...
                template: function (row) {
                    return '\
                        \<a href="' + location.protocol + "//" + window.location.host + '/product-category/edit/' + row._id + '" class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="Edit">\
    ...
    var mealTypesSelector = function () {
        ...
        $(document).on('click', '.KTStatusUpdate', function () {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `${window.location.protocol}//${window.location.host}/product-category/status-change/${elemID}`;
                }
            });
        })
        $(document).on('click', '.ktDelete', function () {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `${location.protocol}//${window.location.host}/product-category/delete/${elemID}`;
                }
            });
        });
    };
