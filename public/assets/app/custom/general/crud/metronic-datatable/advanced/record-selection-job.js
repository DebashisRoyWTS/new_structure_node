...
var KTDatatableMealTypes = function () {
    ...
    var options = {
        ...
        data: {
            ...
            source: {
                read: {
                    url: `${location.protocol}//${window.location.host}/job/getall`,
                },
            },
            pageSize: 10,
            serverPaging: true,
        ...
        columns: [
            ...
            },
            {
                ...
                template: function (row) {
                    return '\
                    \<a href="' + location.protocol + "//" + window.location.host + '/job/edit/' + row._id + '" class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="Edit">\
    ...
    var mealTypesSelector = function () {
        ...
        $('#kt_form_job_type').on('change', function () {
            datatable.search($(this).val(), 'jobType');
        });

        $('#kt_form_status,#kt_form_job_type, #kt_form_workplace').selectpicker();
        ...
        $(document).on('click', '.KTStatusUpdate', function () {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `${window.location.protocol}//${window.location.host}/job/status-change/${elemID}`;
                }
            });
        })
        $(document).on('click', '.ktDelete', function () {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `${location.protocol}//${window.location.host}/job/delete/${elemID}`;
                }
            });
        });
    };
