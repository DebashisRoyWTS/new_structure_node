...
class roleController {
  ...
  async store(req, res) {
   console.log(req.body);
    try {
      req.body.rolegroup = "admin";
      const role_exist = await roleRepo.getByField({
        roleDisplayName: { $regex: req.body.roleDisplayName, $options: "i" },
      });
      if (_.isEmpty(role_exist)) {
        req.body.role = slug(req.body.roleDisplayName, {
          lower: true,
          replacement: "_",
        });
  ...
  async update(req, res) {
    try {
      let role_exist = await roleRepo.getByField({
        roleDisplayName: req.body.roleDisplayName,
        _id: { $ne: req.body.roleId },
      });
      if (_.isEmpty(role_exist)) {
